...
struct TexasHoldemServiceTests {
    ...
    func testPlayersGetUniqueCards() async throws {
        ...
        for (index, player) in game.players.enumerated() {
            print(
                "  Player \(index): \(player.hand.map { "\($0.rank.rawValue)\($0.suit.rawValue)" }.joined(separator: ", "))"
            )
        }
        ...
        for (index, card) in allCards.enumerated() {
            for (otherIndex, otherCard) in allCards.enumerated() {
                if index != otherIndex {
                    ...
                    #expect(
                        !isDuplicate,
                        "Found duplicate card: \(card.rank.rawValue) of \(card.suit.rawValue)")
                }
            }
        }
        ...
        for i in 0..<game.players.count {
            for j in (i + 1)..<game.players.count {
                ...
                let sameHand =
                    hand1.count == hand2.count && hand1[0].rank == hand2[0].rank
                    && hand1[0].suit == hand2[0].suit && hand1[1].rank == hand2[1].rank
        ...
        for iteration in 1...5 {
            ...
            for player in game.players {
                for card in player.hand {
                    ...
                    #expect(
                        !allCards.contains(cardKey),
                        "Duplicate card found in game \(iteration): \(cardKey)")
