...
final class TexasHoldemViewModelSharePlayTests: XCTestCase {
    ...
    func testBetActionBroadcasts() async {
        ...
        XCTAssertTrue(viewModel.sharePlayService.isSessionActive == false, "Not in SharePlay session in tests")
    }
    ...
    func testShowdownBroadcasts() async {
        ...
        XCTAssertEqual(viewModel.currentGame?.gamePhase, .showdown, "Phase should advance to showdown")
    }
    ...
    func testCardsVisibleAtShowdown() async {
        ...
        for player in game.players {
            XCTAssertEqual(player.hand.count, 2, "Each player should have 2 cards visible at showdown")
        }
    }
    ...
    func testSharePlayCallbacksSetCorrectly() {
        ...
        XCTAssertNotNil(viewModel.sharePlayService.onGameStarted, "Game started callback should be set")
        XCTAssertNotNil(viewModel.sharePlayService.onGameStateUpdate, "Game state update callback should be set")
        XCTAssertNotNil(viewModel.sharePlayService.onPlayerAction, "Player action callback should be set")
        XCTAssertNotNil(viewModel.sharePlayService.onPhaseAdvanced, "Phase advanced callback should be set")
    }
}
...
class MockTexasHoldemService: TexasHoldemServiceProtocol {
    ...
    func bet(playerID: UUID, amount: Int) async throws {
        guard var game = currentGame,
              let playerIndex = game.players.firstIndex(where: { $0.userID == playerID }) else {
            return
        }
